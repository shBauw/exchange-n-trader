...
void freeAll(struct trader *traders, char **items, struct orderLevel **orders, int argc, int length, struct counters *counters) {
...
int exited(struct trader *traders, int argc, struct counters *counters) {
	for (int i = 0; i < argc - 2; i++) {
		...
		if (exited == traders[i].pid) {
			...
			counters->exitCounter += 1;
		}
	}
	// If all traders exited, finish trading
	if (counters->exitCounter == argc - 2) {
...
void buy(struct trader *traders, struct orderLevel **orders, char **items, struct counters *counters, int z, int length, char *quantity, char *product, char *price, char *order_id, int argc) {
	...
	if (atoi(quantity) < 1 || atoi(price) < 1 || atoi(quantity) > 999999 || atoi(price) > 999999) invalid(traders[z]);
		...
		for (int i = 0; i < length; i++) {
			if (strcmp(product, items[i]) == 0) {
				...
				for (int k = 0; k < length; k++) counters->fees += balance(orders[k*2], orders[k*2+1], k);
...
void sell(struct trader *traders, struct orderLevel **orders, char **items, struct counters *counters, int z, int length, char *quantity, char *product, char *price, char *order_id, int argc) {
	if (product == NULL || quantity == NULL || price == NULL) {
		invalid(traders[z]);
		return;
	}
...
void amend(struct trader *traders, struct orderLevel **orders, char **items, struct counters *counters, int z, int length, char *quantity, char *price, char *order_id, int argc) {
...
void cancel(struct trader *traders, struct orderLevel **orders, char **items, struct counters *counters, int z, int length, char *order_id, int argc) {
	if (order_id == NULL) {
		invalid(traders[z]);
		return;
	}
...
void check(struct trader *traders, char **items, struct orderLevel **orders, int argc, int length, pid_t tpid, struct counters *counters) {
	int z;
	int parsed = 0;
	char order[200] = {0};
	for (z = 0; z < argc - 2; z++){
	...
	if (strcmp(command, "BUY") == 0) {
		...
		if (atoi(order_id) != traders[z].counter) invalid(traders[z]);
			...
			buy(traders, orders, items, counters, z, length, quantity, product, price, order_id, argc);
		}
	// Sell order
		...
		if (atoi(order_id) != traders[z].counter) invalid(traders[z]);
			...
			sell(traders, orders, items, counters, z, length, quantity, product, price, order_id, argc);
		}
	// Amend order
		...
		amend(traders, orders, items, counters, z, length, quantity, price, order_id, argc);
	// Cancel order
	} else if (strcmp(command, "CANCEL") == 0) {
		char *order_id = strtok(NULL, " ");
		
		cancel(traders, orders, items, counters, z, length, order_id, argc);
	// Invalid if none
	} else {
		invalid(traders[z]);
	}
...
int main(int argc, char ** argv) {
	...
	// Find out how many items, max length of item = 16, rounded to 20.
	char buffer[20];
	fgets(buffer, sizeof(buffer), f);
	int length = atoi(buffer);

	// Allocate memory for items
	char **items = (char **) malloc(sizeof(char *) * length);
	for (int i = 0; i < length; i++) items[i] = (char *) malloc(sizeof(char) * 20);
	
	// Allocate memory for all initial order handlers (one for buy, one for sell)
	struct orderLevel **orders = (struct orderLevel **) malloc(sizeof(struct orderLevel *) * length * 2);
	for (int i = 0; i < length * 2; i++) {
		orders[i] = (struct orderLevel *) malloc(sizeof(struct orderLevel));
		orders[i]->pidAmt = NULL;
		orders[i]->next = NULL;
		orders[i]->prev = NULL;
	}

	// Assign items to memory and print
	printf("[PEX] Trading %d products:", length);
	int i = 0;
	...
	// Allocate space for trader structs, counters
	struct trader *traders = (struct trader *) malloc(sizeof(struct trader) * (argc - 2));
	struct counters *counters = (struct counters *) malloc(sizeof(struct counters));
	counters->fees = 0;
	counters->total = 0;
	counters->exitCounter = 0;
	...
	// Initialise traders
	if (initt(traders, argc, argv, length) == 1) return 1;

	// Send market open to every trader
	for (int i = 0; i < argc - 2; i++) {
		if (traders[i].exited == 1) continue;

		char send[20] = "MARKET OPEN;";
		write(traders[i].exchange, send, strlen(send));
		kill(traders[i].pid, SIGUSR1);
	}

	while (1) {
		// Check if traders have exited
		if (exited(traders, argc, counters) == 0) break;

		// Only executes if a signal is received
		if (pid != 0) {
			pid_t tpid = pid;
			pid = 0;

			check(traders, items, orders, argc, length, tpid, counters);
		}
	}

	// Clean
	freeAll(traders, items, orders, argc, length, counters);
}
